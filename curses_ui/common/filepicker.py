...
def draw_picked_view(win: window, lst: list, selected: int):
    ...
    win.hline(1,0,'-', max_x)
    print_file_list(win, lst=lst, showing=max_y - 4, selected=selected, y_offset=2, x_offset=2)
...
def draw_dir_view(win: window, current: str, lst: list, selected: int):
    ...
    win.hline(1,0,'-', max_x)
    print_file_list(win=win, lst=lst, showing=showing, selected=selected, y_offset=2, x_offset=2)
    win.refresh()
